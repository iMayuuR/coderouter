...
`coderouter-claude` reads the current configured port and auth token, automatically sets the required environment variables (`ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN`, etc.) for the child process, and then launches the real `claude` command. 

**It's that easy!** If the proxy server isn't already running, `coderouter-claude` will smartly start it in the background for you and seamlessly shut it down when you exit Claude. You don't need to manage multiple terminals.
...
For terminal use, simply run the installed launcher:

```bash
coderouter-claude
```

**That's all you need!** The launcher automatically sets all required environment variables and smartly starts the `coderouter-server` proxy in the background if it isn't already running. It will even cleanly shut down the background server when you exit Claude. You can also run `coderouter-server` manually if you prefer keeping it in a separate terminal.
...
Config precedence is repo `.env`, then `~/.config/coderouter/.env`, then `FCC_ENV_FILE` when set. `ANTHROPIC_AUTH_TOKEN` can be any local secret; pass the same value to Claude Code.
...
- Report bugs and feature requests in [Issues](https://github.com/iMayuuR/coderouter/issues).
