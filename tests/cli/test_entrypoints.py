...
def _run_init(tmp_home: Path) -> tuple[str, Path]:
    ...
    env_file = tmp_home / ".config" / "coderouter" / ".env"
    printed: list[str] = []
...
def test_init_creates_parent_directories(tmp_path: Path) -> None:
    """init() creates ~/.config/coderouter/ even if it doesn't exist."""
    config_dir = tmp_path / ".config" / "coderouter"
...
def test_init_skips_if_env_already_exists(tmp_path: Path) -> None:
    ...
    env_file = tmp_path / ".config" / "coderouter" / ".env"
...
def test_init_prints_next_step_hint(tmp_path: Path) -> None:
    """init() tells the user to run coderouter-server after editing .env."""
    output, _ = _run_init(tmp_path)

    assert "coderouter-server" in output
...
def test_cli_scripts_are_registered() -> None:
    ...
    assert scripts["coderouter-server"] == "cli.entrypoints:serve"
    assert scripts["coderouter-claude"] == "cli.entrypoints:launch_claude"
...
def test_claude_child_env_removes_blank_configured_auth_token() -> None:
    ...
    assert env["ANTHROPIC_API_KEY"] == "sk-ant-dummy-coderouter"
...
def test_launch_claude_auto_starts_proxy_and_terminates_it() -> None:
    from cli.entrypoints import launch_claude
    ...
    kill_tree.assert_called_once_with(9999)
    proxy_proc.wait.assert_called_once_with(timeout=5)
...
def test_launch_claude_unreachable_proxy_auto_start_fails_exits(
    capsys: pytest.CaptureFixture[str],
) -> None:
    from cli.entrypoints import launch_claude
