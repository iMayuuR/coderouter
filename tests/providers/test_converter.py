...
def test_convert_user_message_image_success():
    content = [
        MockBlock(type="text", text="Here is my query:"),
        MockBlock(
            type="image",
            source={"type": "base64", "media_type": "image/jpeg", "data": "abc123xyz"},
        ),
    ]
    messages = [MockMessage("user", content)]
    result = AnthropicToOpenAIConverter.convert_messages(messages)
    ...
    assert result[0]["content"][0] == {"type": "text", "text": "Here is my query:"}
    assert result[0]["content"][1] == {
        "type": "image_url",
        "image_url": {"url": "data:image/jpeg;base64,abc123xyz"},
    }
