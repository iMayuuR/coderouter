...
def _env_files() -> tuple[Path, ...]:
    ...
    files: list[Path] = [
        Path(".env"),
        Path.home() / ".config" / "coderouter" / ".env",
        Path.home() / ".config" / "coderouter" / ".env",
    ]
    if explicit := os.environ.get("FCC_ENV_FILE"):
        files.append(Path(explicit))
    if explicit := os.environ.get("CODEROUTER_ENV_FILE"):
        files.append(Path(explicit))
    return tuple(files)
...
class Settings(BaseSettings):
    ...
    # ==================== Google Stitch MCP Config ====================
    google_stitch_mcp_api_key: str = Field(
        default="", validation_alias="GOOGLE_STITCH_MCP_API_KEY"
    )

    # ==================== 21st Dev Config ====================
    twentyone_dev_api_key: str = Field(
        default="", validation_alias="TWENTYONE_DEV_API_KEY"
    )
