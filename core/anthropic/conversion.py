...
class AnthropicToOpenAIConverter:
    ...
    @staticmethod
    def _build_openai_user_content(blocks: list[Any]) -> str | list[dict[str, Any]]:
        has_image = any(get_block_type(b) == "image" for b in blocks)
        if not has_image:
            text_parts = [
        ...
        for b in blocks:
            ...
            if b_type == "text":
                current_text_parts.append(get_block_attr(b, "text", ""))
            elif b_type == "image":
                flush_text_parts()
                source = get_block_attr(b, "source")
    ...
    def _convert_user_message_with_injection(
        ...
        def flush_user_msg() -> None:
            if current_user_blocks:
                content_val = AnthropicToOpenAIConverter._build_openai_user_content(
                    current_user_blocks
                )
