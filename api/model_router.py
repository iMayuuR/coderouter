...
def _request_has_image(request: MessagesRequest | TokenCountRequest) -> bool:
...
class ModelRouter:
    ...
    def resolve_messages_request(
        ...
        if _request_has_image(request) and not _is_multimodal_model(
            resolved.provider_model
        ):
            # Try to find a configured model that is multimodal
            multimodal_ref = None
            for ref_name in [
                ...
                if ref_name:
                    model_name = Settings.parse_model_name(ref_name)
                    if _is_multimodal_model(model_name):
                        multimodal_ref = ref_name
                        break

            if multimodal_ref:
                logger.info(
                    "Image detected in request but resolved model '{}' is not multimodal. "
                    "Auto-routing to multimodal model '{}'.",
                    resolved.provider_model_ref,
                    multimodal_ref,
                )
                resolved = self.resolve(multimodal_ref)
    ...
    def resolve_token_count_request(
        ...
        if _request_has_image(request) and not _is_multimodal_model(
            ...
            if multimodal_ref:
                logger.info(
                    "Image detected in token-count request but resolved model '{}' is not multimodal. "
                    "Auto-routing to multimodal model '{}'.",
                    resolved.provider_model_ref,
                    multimodal_ref,
                )
