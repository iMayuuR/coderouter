...
def cmd_import(args) -> int:
    ...
    if target_scope == "global":
        ...
        existing += _load_instincts_from_dir(
            GLOBAL_INHERITED_DIR, "inherited", "global"
        )
    else:
    ...
    for inst in new_instincts:
        inst_id = inst.get("id")
        if inst_id not in best_by_id or inst.get("confidence", 0.5) > best_by_id[
            inst_id
        ].get("confidence", 0.5):
    ...
    for inst in to_update:
        ...
        if stale and stale.get("_source_file"):
            ...
            if stale_path.exists() and str(stale_path).startswith(
                str(scope_root) + os.sep
            ):
...
def cmd_evolve(args) -> int:
    ...
    if args.generate:
        evolved_dir = (
            project["evolved_dir"] if project["id"] != "global" else GLOBAL_EVOLVED_DIR
        )
...
def _promote_specific(
    project: dict, instinct_id: str, force: bool, dry_run: bool = False
) -> int:
    ...
    if dry_run:
        if not quiet:
            if expired:
                ...
                for item in expired:
                    print(
                        f"  - {item['name']} (age: {item['age_days']}d) — {item['path']}"
                    )
            else:
...
def main() -> int:
    ...
    prune_parser = subparsers.add_parser(
        "prune", help="Delete pending instincts older than TTL"
    )
