...
class NetworkGraphExplainer(Scene):
    def construct(self):
        ...
        subtitle = Text(
            "Prune low-signal follows. Strengthen warm paths.", font_size=20
        ).next_to(title, DOWN)
        ...
        new_target = Circle(radius=0.42, color="#FF9F1C").shift(
            RIGHT * 3.0 + DOWN * 1.4
        )
        ...
        edge_stale_a = CurvedArrow(
            you.get_right(), stale_a.get_left(), angle=0.2, color="#7A7A7A"
        )
        edge_stale_b = CurvedArrow(
            you.get_right(), stale_b.get_left(), angle=-0.2, color="#7A7A7A"
        )
        edge_bridge = CurvedArrow(
            you.get_right(), bridge.get_left(), angle=0.0, color="#21A179"
        )
        edge_target = CurvedArrow(
            bridge.get_right(), target.get_left(), angle=0.1, color="#21A179"
        )
        edge_new_target = CurvedArrow(
            bridge.get_right(), new_target.get_left(), angle=-0.12, color="#21A179"
        )
        ...
        self.play(
            FadeOut(stale_a),
            FadeOut(stale_b),
            FadeOut(stale_a_label),
            FadeOut(stale_b_label),
            FadeOut(edge_stale_a),
            FadeOut(edge_stale_b),
        )
